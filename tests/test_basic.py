...
import viprs as vp
from viprs.model import VIPRS, VIPRSGrid, VIPRSMix
...
class TestVIPRS(object):
    def test_init(self, viprs_model: VIPRS, gdl_object: mgp.GWADataLoader):
        ...
        assert 0.0 < viprs_model.pi < 1.0
        assert 0.0 < viprs_model.sigma_epsilon < 1.0
        assert viprs_model.tau_beta > 0.0

        # Check the model parameters:
        for p in (
            viprs_model.var_gamma,
            viprs_model.var_mu,
            viprs_model.var_tau,
            viprs_model.q,
            viprs_model.eta,
    ...
    def test_fit(self, viprs_model: VIPRS):
        ...
        for p in (
            viprs_model.pip,
            viprs_model.post_mean_beta,
            viprs_model.post_var_beta,
        ):
            assert p[22].shape == (viprs_model.m,)
...
class TestVIPRSMix(TestVIPRS):
    def test_init(self, viprsmix_model: VIPRSMix, gdl_object: mgp.GWADataLoader):
        ...
        assert np.all((0.0 < viprsmix_model.pi) & (viprsmix_model.pi < 1.0))
        assert 0.0 < np.sum(viprsmix_model.pi) < 1.0
        assert 0.0 < viprsmix_model.sigma_epsilon < 1.0
        assert np.all(viprsmix_model.tau_beta > 0.0)

        # Check the variational parameters:
        for p in (
            viprsmix_model.var_gamma,
            viprsmix_model.var_mu,
            viprsmix_model.var_tau,
        ):
    ...
    def test_fit(self, viprsmix_model):
        ...
        for p in (
            viprsmix_model.pip,
            viprsmix_model.post_mean_beta,
            viprsmix_model.post_var_beta,
        ):
            assert p[22].shape == (viprsmix_model.m,)
...
class TestVIPRSGrid(TestVIPRS):
    ...
    def test_fit(self, viprs_grid_model: VIPRSGrid):
        ...
        for p in (
            viprs_grid_model.std_beta,
            viprs_grid_model.validation_std_beta,
            viprs_grid_model.n_per_snp,
        ):
        ...
        for criterion in model_selection_criteria:
            ...
            for p in (
                viprs_grid_model.pip,
                viprs_grid_model.post_mean_beta,
                viprs_grid_model.post_var_beta,
            ):
