...
def measure_e_step_performance(
    viprs_model, n_experiments=15, n_calls=None, warm_up=10, initialize_once=False
):
    """
    ...
    if n_calls is None:
        ...
        time_iter = math.ceil(
            1.0
            / np.min(timeit.repeat(exec_func, repeat=5 + warm_up, number=5)[warm_up:])
        )
        n_calls = 5 * int(time_iter)

    with ResourceProfiler(dt=0.1) as rprof:
        times = timeit.repeat(
            exec_func, repeat=n_experiments + warm_up, number=int(n_calls)
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Benchmark the speed of the E-Step in VIPRS"
    )
    ...
    parser.add_argument(
        "--sumstats-format", dest="sumstats_format", type=str.lower, default="fastgwa"
    )
    parser.add_argument(
        "--model",
        dest="model",
    ...
    parser.add_argument(
        "--low-memory", dest="low_memory", default=False, action="store_true"
    )
    parser.add_argument(
        "--threads",
        dest="threads",
    ...
    parser.add_argument(
        ...
        help="The float precision to use for VIPRS.",
    )
    parser.add_argument(
        "--initialize-once",
        dest="initialize_once",
        default=False,
        action="store_true",
    ...
    if not blas_supported and args.linalg == "blas":
        raise ValueError(
            "BLAS is not supported on this platform to benchmark its performance."
        )
    ...
    general_info = {
        ...
        "median_n_neighbors": np.median(
            np.concatenate([ld.window_size for ld in gdl.ld.values()])
        ),
        "avg_n_neighbors": np.mean(
            np.concatenate([ld.window_size for ld in gdl.ld.values()])
        ),
    ...
    for t in n_threads:
        print(
            f"Experiment details --> Float precision {args.float_precision} | "
        ...
        if args.model in ("VIPRS", "ALL"):
            if args.implementation in ("cython", "all"):
                if args.linalg in ("manual", "all"):
                    ...
                    res = measure_e_step_performance(
                        v,
        ...
        output_fname = (
            f"{args.file_prefix}timing_results_imp{args.implementation}_model{args.model}_"
