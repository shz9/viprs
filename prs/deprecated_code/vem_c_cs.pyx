...
cdef class vem_prs_cs(PRSModel):
    """
    This implementation imitates the Carbonetto & Stephens (2012) model formulation
    """
    ...
    def initialize_variational_params(self):
        ...
        for c, c_size in self.shapes.items():
            ...
            self.var_gamma[c] = np.repeat(0.5, c_size)
    ...
    def e_step(self):
        ...
        cdef double denom = (1. + 1./(self.N * self.sigma_beta))
        ...
        for c, c_size in self.shapes.items():
            ...
            self.var_sigma_beta[c] = np.repeat(
                self.sigma_epsilon / (self.N + 1./self.sigma_beta),
                c_size
            )
            ...
            for i, Di in enumerate(self.ld[c].iterate()):
                ...
                u_i = (log_pi + .5*log(var_sigma_beta[i] / (self.sigma_epsilon*self.sigma_beta)) +
    ...
    def m_step(self):
        ...
        if 'sigma_beta' not in self.fix_params:
            ...
            self.sigma_beta = (np.sum([
                ...
                for c in self.var_mu_beta])) / (self.sigma_epsilon * var_gamma_sum)
            """
            self.sigma_beta = (10 + np.sum([
                np.dot(self.var_gamma[c],
                       self.var_mu_beta[c]**2 + self.var_sigma_beta[c])
                for c in self.var_mu_beta])) / (10 + self.sigma_epsilon*var_gamma_sum)
            """
        ...
        for c, c_size in self.shapes.items():
            ...
            for i, Di in enumerate(self.ld[c].iterate()):
                ...
                sig_e += .5*(1. + 1./(self.N*self.sigma_beta))*var_gamma[i]*(var_mu_beta[i]*var_mu_beta[i] + var_sigma_beta[i])
        ...
        if 'sigma_epsilon' not in self.fix_params:
            final_sig_e = (self.N / (self.N + var_gamma_sum))*(1. + 2.*sig_e)
    ...
    def objective(self):
        ...
        loglik -= .5 * self.M * np.log(2. * np.pi * self.sigma_beta * self.sigma_epsilon)
        loglik += self.M * np.log(1. - self.pi)

        ent += .5 * self.M * np.log(2. * np.pi * np.e * self.sigma_beta * self.sigma_epsilon)
        ...
        for c in self.var_mu_beta:
            ...
            loglik += (-.5 / (self.sigma_epsilon*self.sigma_beta)) * (
                    np.sum(gamma_mu_sig) +
                    self.sigma_beta * self.sigma_epsilon * np.sum(1 - self.var_gamma[c])
            )

            loglik += np.log(self.pi / (1. - self.pi)) * np.sum(self.var_gamma[c])

            ent += .5 * np.dot(self.var_gamma[c], np.log(self.var_sigma_beta[c] / (self.sigma_epsilon*self.sigma_beta)))
